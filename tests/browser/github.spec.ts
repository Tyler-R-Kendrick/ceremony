...
test("legacy live GitHub adapter exposes blocking prerequisites and a real manifest scenario, without granting access", async ({
  page,
  context,
}) => {
  ...
  try {
    ...
    const snapshot = await (
      await page.request.get(`${app.origin}/api/live/ceremonies/${id}`)
    ).json();
    const bypass = await page.request.post(
      `${app.origin}/api/live/ceremonies/${id}/actions`,
      {
    ...
    expect(sent.url).toBe(app.origin);
