...
async function fixture(
  t: TestContext,
  options: { verified?: boolean; handler?: () => Promise<void> } = {},
) {
  ...
  const registry = new OperationRegistry(
    new Map([["verified", { classification: "public", schema: z.boolean() }]]),
  );
  for (const id of ["prepare", "verify"])
    registry.register({
      contract: {
        ...
        profile: "app",
        inputs: {},
        outputs: { verified: { contract: "verified", required: true } },
        effects: ["read"],
        verifier: "provider",
        humanFallback: "provider-consent",
      },
      inputSchema: z.strictObject({}),
      outputSchema: z.strictObject({ verified: z.boolean() }),
      classifications: {},
      fixtures: ["signed-local-provider"],
      handler: async () => {
        effects++;
        await options.handler?.();
        return { state: "complete", outputs: { verified: true } };
      },
...
test("AC-16 AC-27: protected commands reject unmet dependencies and deduplicate concurrent commands", async (t) => {
  ...
  await assert.rejects(
    f.commands.advance(actor, f.run.id, "verify", 1, "forged"),
  );
  assert.equal(f.effects(), 0);
  const results = await Promise.all([
    f.commands.advance(actor, f.run.id, "prepare", 1, "same"),
    f.commands.advance(actor, f.run.id, "prepare", 1, "same"),
  ]);
  ...
  await assert.rejects(
    f.commands.advance(actor, f.run.id, "verify", 2, "same"),
  );
  await assert.rejects(
...
test("AC-07 AC-10 AC-35: authoritative recording consent and durable continuation survive without UI callbacks", async (t) => {
  ...
  const handlers = new Map([
    [
      ...
      async ({ deliveryId }: { deliveryId: string }) => {
        deliveries++;
        seen.add(deliveryId);
        if (deliveries === 1) throw new Error("ack lost");
      },
    ],
  ]);
  ...
  assert.ok(
    (await f.store.transaction((tx) => tx.list(actor.tenantId, "audit")))
      .length > 0,
  );
});
...
test("AC-18 AC-33: revocation and cancellation during an external call fence late results", async (t) => {
  ...
  assert.equal(
    (await f.commands.snapshot(actor, f.run.id)).nodes[0]!.verified,
    false,
  );
  f.revoke();
  await assert.rejects(
    f.commands.advance(actor, f.run.id, "prepare", 2, "revoked"),
  );
  assert.equal(f.effects(), 1);
});
...
test("AC-29: an unverified handler response or uncertain effect cannot satisfy a dependency", async (t) => {
  ...
  const result = await f.commands.advance(
    actor,
    f.run.id,
    "prepare",
    1,
  ...
  await assert.rejects(
    f.commands.advance(actor, f.run.id, "verify", 2, "downstream"),
  );
  assert.equal(f.effects(), 1);
  assert.equal(
    JSON.stringify(await f.commands.snapshot(actor, f.run.id)).includes(
      "outputs",
    ),
    false,
  );
...
test("AC-27: a completed child cannot repeat under a new command ID while its parent remains active", async (t) => {
  const f = await fixture(t);
  const first = await f.commands.advance(
    actor,
    f.run.id,
    "prepare",
    1,
  ...
  const current = await f.commands.snapshot(actor, f.run.id);
  ...
  assert.equal(
    (await f.commands.advance(actor, f.run.id, "prepare", 1, "first-command"))
      .verified,
    true,
  );
  assert.equal(f.effects(), 1);
  await f.commands.advance(
    actor,
    f.run.id,
    "verify",
    current.revision,
