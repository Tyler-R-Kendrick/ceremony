...
export async function startWorkOsPipesDouble(
  ...
  const fixture = await startHttpFixture(
    (request: RecordedRequest): FixtureReply => {
      ...
      if (authorize && request.method === "POST") {
        ...
        const owner =
          typeof body.connection_owner === "string"
            ? body.connection_owner
            : "user";
        if (
          owner === "organization" &&
      ...
      if (userAccount || orgAccount) {
        ...
        const provider = decodeURIComponent((orgAccount ?? userAccount)![2]!);
      ...
      /* ----------------------------------------------- token / credentials */
      const vend = /^\/data-integrations\/([^/]+)\/(token|credentials)$/.exec(
        path,
      );
      if (vend && request.method === "POST") {
        ...
        const connection = connections.get(
          key(
            provider,
            owner as "user" | "organization",
            userId,
            organizationId,
          ),
        );
        if (!connection)
          return json(200, { active: false, error: "not_installed" });
      ...
      const orgProviders = /^\/organizations\/([^/]+)\/data_providers$/.exec(
        path,
      );
      ...
      if (path === "/relay" || path.startsWith("/relay/")) {
        ...
        if (path === "/relay") {
          ...
          if (headerProvider && headerProvider !== slug)
            return json(400, {
              ...
              message:
                "The path slug and X-Relay-Provider name different providers.",
            });
        }
        if (!slug)
          return json(404, {
        ...
        const candidate =
          ...
          (relayOrganization
            ? connections.get(
                key(slug, "organization", relayUser, relayOrganization),
              )
            : undefined);
        const usable =
          candidate &&
