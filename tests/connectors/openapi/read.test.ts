...
const operation = (read: Awaited<ReturnType<typeof readFixture>>, id: string) => {
...
test("the version field alone selects the reader", async () => {
  assert.deepEqual(detectOpenApiVersion(fixture("swagger-2.0-inventory.json")), {
    profile: "swagger-2.0",
    version: "2.0",
  });
...
test("a missing, unknown or ambiguous version is refused rather than guessed", async () => {
  ...
  const both = await readOpenApi({ ...shapedLike31, swagger: "2.0", openapi: "3.1.0" });
  ...
  for (const value of ["3.1", "v3.1.0", "", 3.1, null])
    assert.equal((await readOpenApi({ ...shapedLike31, openapi: value })).profile, undefined);
});
...
test("Swagger 2.0: body, formData and collectionFormat are translated exactly", async () => {
  ...
  assert.ok(upload.parameters.some((parameter) => parameter.name === "itemId" && parameter.in === "path"));
});
...
test("OpenAPI 3.0: requestBody, styles and server variables are preserved", async () => {
  ...
  const trace = list.parameters.find((parameter) => parameter.name === "X-Request-Trace");
  ...
  assert.equal(session.parameters.find((parameter) => parameter.name === "session")?.in, "cookie");
});
...
test("OpenAPI 3.0: callbacks become event descriptors without becoming executable", async () => {
  ...
  const event = read.definition.events.find((item) => item.label === "invoicePaid");
  assert.ok(event);
  assert.equal(event.transport, "http-webhook");
  assert.equal(event.verification, "unknown");
  // A callback is not an operation.
  assert.ok(!read.definition.capabilities.some((item) => item.nativeId === "invoicePaidCallback"));
});
...
test("OpenAPI 3.1: webhooks, dialect and type arrays are read", async () => {
  ...
  assert.ok(read.definition.events.some((event) => event.transport === "http-webhook"));
  // Webhooks are events, never invocable capabilities.
  assert.ok(!read.definition.capabilities.some((item) => item.nativeId === "productChangedWebhook"));
});
...
test("OpenAPI 3.2: QUERY, additionalOperations and querystring are read", async () => {
  ...
  assert.equal(
    operation(read, "searchRaw").parameters.find((parameter) => parameter.name === "rawQuery")?.in,
    "querystring",
  );
  // A 3.2-only path-item key is not read by the 3.1 reader.
  const as31 = readOpenApi31({ ...(fixture("openapi-3.2-fleet.json") as object), openapi: "3.1.0" });
...
test("a duplicate operationId does not collide: the second operation keeps a distinct identity", async () => {
  ...
  assert.ok(read.issues.some((issue) => issue.code === "structure.operation-id-duplicate"));
});
...
test("a recursive schema is preserved without infinite expansion", async () => {
  ...
  assert.ok(read.definition.capabilities.some((item) => item.nativeId === "getTree"));
  assert.ok(read.definition.capabilities.some((item) => item.nativeId === "putPerson"));
...
test("operation effect is read for GET and HEAD and stays unknown otherwise", async () => {
  const read = await readFixture("openapi-3.1-catalog.json");
  const byId = new Map(read.definition.capabilities.map((item) => [item.nativeId, item]));
...
test("x-* extensions are preserved inertly and example-bearing keys are excluded", async () => {
  ...
  const list = read.definition.capabilities.find((item) => item.nativeId === "ListTickets");
  ...
  const queue = operation(read, "ListTickets").parameters.find((item) => item.name === "queue");
...
test("securityRequirementsFor reports alternatives, AND, inheritance and anonymity", async () => {
  ...
  assert.deepEqual(list.alternatives[0]?.schemes.map((entry) => entry.scheme), ["api_key"]);
  ...
  assert.deepEqual(create.alternatives[0]?.schemes[0]?.scopes, ["inventory:write"]);
...
test("the reader produces a definition whose digest is stable across reads", async () => {
  ...
  assert.equal(first.definition.normalizedDigest, second.definition.normalizedDigest);
...
test("import never claims invoke: the definition's dimensions require configuration", async () => {
  const read = await readFixture("openapi-3.1-catalog.json");
  assert.equal(read.definition.compatibility.dimensions.invoke, "requires-configuration");
  assert.equal(read.definition.compatibility.dimensions.discover, "unsupported");
