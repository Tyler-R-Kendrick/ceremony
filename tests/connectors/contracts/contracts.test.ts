...
test("CON-05-01: severity, disposition and execution impact agree; security is never talked down", () => {
  ...
  const invalid: Array<[Partial<CompatibilityIssue>, string]> = [
    [
      ...
      "Unsupported security requirements are blocking",
    ],
    [
      {
        category: "security",
    ...
    [
      ...
      "An unconfigured security requirement names what it blocks",
    ],
    [
      {
        category: "security",
    ...
    [
      ...
      "A blocking issue must name what it blocks",
    ],
    [
      {
        category: "structure",
      ...
      "An informational issue blocks nothing",
    ],
    [
      {
        category: "structure",
      ...
      "Blocking the definition is a blocking severity",
    ],
    [
      {
        category: "structure",
      ...
      "A mapped construct cannot also be blocking",
    ],
    [
      {
        category: "structure",
    ...
    [
      ...
      "A rejected construct is never informational",
    ],
  ];
  for (const [change, message] of invalid) {
    ...
    assert.ok(
      messages(result).includes(message),
      `${JSON.stringify(change)} -> ${messages(result)}`,
    );
  }
  for (const change of [
    { code: "Bad.Code" },
    ...
    rejects(
      compatibilityIssueSchema,
      issue(change as Partial<CompatibilityIssue>),
      JSON.stringify(change),
    );
});
...
test("CON-01-CS: capability statuses keep implementation, configuration and evidence honest", () => {
  ...
  assert.ok(
    capabilityStatusSchema.safeParse(
      status({ implementation: "unsupported", evidence: "not-tested" }),
    ).success,
  );
  assert.ok(
    capabilityStatusSchema.safeParse(
      status({ configuration: "missing", evidence: "browser-integration" }),
    ).success,
  );
  assert.ok(
    capabilityStatusSchema.safeParse(
      status({
        configuration: "ready",
        evidence: "live-authorized",
        evidenceRef: "evidence:live-1",
      }),
    ).success,
  );
  assert.ok(
    capabilityStatusSchema.safeParse(
      status({
        ...
        evidence: "deployed-authorized",
      }),
    ).success,
  );
  assert.equal(
    capabilityStatusSchema.parse(status({ adapterVersion: "2026-07-28" }))
      .adapterVersion,
    "2026-07-28",
  );
  const invalid: Array<[Record<string, unknown>, string]> = [
    [
      { implementation: "unsupported", evidence: "unit" },
      "An unsupported dimension has no passing evidence",
    ],
    [
      { evidence: "not-tested", evidenceRef: "evidence:1" },
      "Evidence reference requires an evidence level",
    ],
    [
      { configuration: "missing", evidence: "live-authorized" },
      "Live evidence requires the configuration it was measured with",
    ],
    [
      { configuration: "missing", evidence: "deployed-authorized" },
      "Live evidence requires the configuration it was measured with",
    ],
  ];
  for (const [change, message] of invalid) {
...
test("CON-03-02: requested, reported and observed permissions stay distinct and empty means unknown", () => {
  ...
  assert.ok(
    messages(observedNothing).includes(
      "An observed-permission claim lists the permissions it observed",
    ),
  );
  ...
  rejects(
    verificationClaimSchema,
    { ...withoutPermissions, kind: "permission-observed" },
    "permission-observed without permissions",
  );
  assert.ok(
...
test("CON-03-03: issuers observe what they can observe; host policy asserts ownership only", () => {
  ...
  for (const [issuer, kind, expected] of combos) {
    ...
    assert.equal(
      verificationClaimSchema.safeParse(claim).success,
      expected,
      `${issuer} ${kind}`,
    );
  }
...
test("CON-03-04: validity compares instants, not strings, so offsets cannot fake a window", () => {
  assert.ok(
    verificationClaimSchema.safeParse(
      buildClaim({
        observedAt: "2026-09-18T02:00:00+02:00",
        validUntil: "2026-09-18T00:30:00Z",
      }),
    ).success,
  );
  ...
  for (const [observedAt, validUntil] of windows)
    rejects(
      verificationClaimSchema,
      { ...buildClaim(), observedAt, validUntil },
...
test("CON-01-CN: connection summaries carry no credentials and keep target, handoff and time consistent", () => {
  ...
  assert.ok(
    connectionSummarySchema.safeParse(buildConnectionSummary({ service: "" }))
      .success,
  );
  assert.ok(
    connectionSummarySchema.safeParse({
      ...summary,
      handoff: buildHandoffSummary({ generation: 0 }),
    }).success,
  );
  const {
  ...
  const invalid: Array<[Record<string, unknown>, string]> = [
    [
      { ...summary, createdAt: LATER, updatedAt: AT },
      "A connection is updated after it is created",
    ],
    [
      { ...unverified, target: summary.target },
      "A verified target requires verification evidence",
    ],
    [
      { ...summary, verification: { ...summary.verification, validUntil: AT } },
      "Verification validity ends after it was observed",
    ],
    [
      {
        ...summary,
      ...
      "A handoff belongs to the current connection generation",
    ],
  ];
  for (const [candidate, message] of invalid) {
    const result = connectionSummarySchema.safeParse(candidate);
  ...
  for (const change of [
    ...
    { externalIds: { connectionId: "abc" } },
    {
      handoff: {
        ...buildHandoffSummary(),
        url: "https://example.invalid/continue",
      },
    },
    { lifecycle: "connected" },
  ])
...
test("CON-01-LC: lifecycle transitions enter active only through verification or reconciliation", () => {
  ...
  assert.deepEqual(
    Object.keys(lifecycleTransitions).sort(),
    [...connectionLifecycles].sort(),
  );
  for (const [from, targets] of Object.entries(lifecycleTransitions))
    for (const to of targets) {
      assert.ok(
        (connectionLifecycles as readonly string[]).includes(to),
        `${from} -> ${to}`,
      );
      assert.notEqual(from, to);
    }
  const intoActive = connectionLifecycles.filter(
    (from) => from !== "active" && canTransitionLifecycle(from, "active"),
  );
  assert.deepEqual(intoActive.sort(), [
    "degraded",
    "indeterminate",
    "verifying",
  ]);
  for (const from of connectionLifecycles) {
    ...
    if (from !== "locally-disconnected")
      assert.ok(
        canTransitionLifecycle(from, "locally-disconnected"),
        `${from} -> local disconnect`,
      );
  }
  for (const from of [
  ...
  ] as ConnectionLifecycle[])
    assert.equal(
      canTransitionLifecycle(from, "active"),
      false,
      `${from} -> active`,
    );
  assert.equal(
    canTransitionLifecycle("locally-disconnected", "verifying"),
    false,
  );
});
...
test("CON-01-CE: catalog entries cannot outrank their evidence, hide missing configuration or fake implementation", () => {
  ...
  const missingConfiguration = entry.configuration.map((item) => ({
    ...item,
    present: false,
  }));
  assert.ok(
  ...
  assert.ok(
    catalogEntrySchema.safeParse({ ...entry, support: "provider-backed" })
      .success,
  );
  assert.ok(
    catalogEntrySchema.safeParse({
  ...
  assert.ok(
    catalogEntrySchema.safeParse({
      ...
      capabilities: [
        buildCapabilityStatus(),
        buildCapabilityStatus({ profile: "openapi-3.0" }),
      ],
    }).success,
  );
  const invalid: Array<[Record<string, unknown>, string]> = [
    [
      { evidence: "local-integration" },
      "Entry evidence cannot exceed its strongest measured dimension",
    ],
    [
      { capabilities: [], evidence: "unit" },
      "Entry evidence cannot exceed its strongest measured dimension",
    ],
    [
      { support: "provider-backed", configuration: missingConfiguration },
      "A provider-backed entry lacking required configuration is unconfigured",
    ],
    [
      { support: "unconfigured" },
      "An unconfigured entry lacks some required configuration",
    ],
    [
      {
        support: "fixture",
        evidence: "live-authorized",
        capabilities: [buildCapabilityStatus({ evidence: "live-authorized" })],
      },
      "Fixture and catalog entries carry no live evidence",
    ],
    [
      {
        support: "catalog-only",
        evidence: "not-tested",
        capabilities: [buildCapabilityStatus({ evidence: "not-tested" })],
      },
      "A catalog-only entry implements nothing",
    ],
    [{ custody: ["host-owned", "host-owned"] }, "Duplicate custody entry"],
    [
      { runtimes: ["hosted-server", "hosted-server"] },
      "Duplicate runtime entry",
    ],
    [{ authentication: ["none", "none"] }, "Duplicate authentication entry"],
    [
      { configuration: [entry.configuration[0], entry.configuration[0]] },
      "Duplicate configuration entry",
    ],
    [
      { capabilities: [buildCapabilityStatus(), buildCapabilityStatus()] },
  ...
  for (const change of [
    {
      configuration: [
        { ...entry.configuration[0], value: "CANARY_CONFIG_VALUE_5e6" },
      ],
    },
...
test("CON-01-SR: source records keep provenance chains honest and locations free of credentials", () => {
  ...
  const step = (inputDigest: string, outputDigest: string) => ({
    step: "overlay",
    version: "1.1.0",
    inputDigest,
    outputDigest,
  ...
  assert.ok(
    sourceRecordSchema.safeParse({
      ...source,
      adaptation: [step(HEX, HEX_2), step(HEX_2, HEX)],
    }).success,
  );
  const broken = sourceRecordSchema.safeParse({
    ...source,
    adaptation: [step(HEX, HEX_2), step(HEX, HEX_2)],
  });
  assert.equal(broken.success, false);
  assert.ok(messages(broken).includes("Adaptation steps form a digest chain"));
  assert.ok(
    sourceRecordSchema.safeParse({ ...source, origin: { kind: "upload" } })
      .success,
  );
  for (const change of [
    { origin: { kind: "url" } },
    {
      origin: {
        kind: "url",
        location: "https://user:CANARY_PASS_7c1@example.invalid/openapi.json",
      },
    },
    {
      origin: {
        kind: "url",
        location:
          "https://example.invalid/openapi.json?token=CANARY_SECRET_9f3",
      },
    },
    {
      origin: {
        kind: "url",
        location: "https://example.invalid/openapi.json#frag",
      },
    },
...
test("CON-01-DF: definitions state every dimension, resolve capabilities exactly and carry no executable fields", () => {
  ...
  const partial = normalizedDefinitionSchema.safeParse({
    ...definition,
    compatibility: { issues: [], dimensions: { import: "exact" } },
  });
  assert.equal(partial.success, false);
  assert.ok(
    normalizedDefinitionSchema.safeParse({
      ...definition,
      compatibility: { issues: [], dimensions: completeDimensions({}) },
    }).success,
  );
  rejects(
    normalizedDefinitionSchema,
    {
      ...
      compatibility: {
        issues: [],
        dimensions: { ...completeDimensions({}), execute: "exact" },
      },
    },
    "unknown dimension",
  );
  ...
  assert.ok(
    normalizedDefinitionSchema.safeParse({
      ...definition,
      capabilities: [definition.capabilities[0], second],
    }).success,
  );
  const invalid: Array<[Record<string, unknown>, string]> = [
    [
    ...
    [
      { capabilities: [buildCapability({ authentication: ["missing"] })] },
      "Capability references an unknown authentication profile",
    ],
    [
      {
        configuration: [
          definition.configuration[0],
          definition.configuration[0],
        ],
      },
    ...
    [
      { capabilities: [definition.capabilities[0], buildCapability()] },
  ...
  for (const [change, message] of invalid) {
    const result = normalizedDefinitionSchema.safeParse({
      ...definition,
      ...change,
    });
    assert.equal(result.success, false, message);
  ...
  assert.ok(
    normalizedDefinitionSchema.safeParse({
      ...definition,
      declaredServers: [
        { url: "https://{region}.api.example/v1", status: "declared" },
      ],
    }).success,
  );
  assert.ok(
  ...
  for (const change of [
    {
      declaredServers: [
        {
          url: "https://files.example.invalid/doc?X-Amz-Signature=CANARY_SIG_8e5",
          status: "declared",
        },
      ],
    },
    {
      declaredServers: [
        {
          url: "https://user:CANARY_PASS_7c1@example.invalid/path",
          status: "declared",
        },
      ],
    },
    {
      declaredServers: [
        { url: "https://api.example/v1#frag", status: "declared" },
      ],
    },
    {
    ...
    },
    {
      authentication: [
        buildProfile("oauth-authorization-code", {
          tokenEndpoint: "https://auth.example/token?key=CANARY_SECRET_9f3",
        }),
      ],
    },
    {
      authentication: [
        buildProfile("oauth-authorization-code", {
          issuer: "https://auth.example/#frag",
        }),
      ],
    },
    {
    ...
    },
    {
      authentication: [
        buildProfile("oauth-authorization-code", {
          clientSecret: "CANARY_SECRET_9f3",
        }),
      ],
    },
    {
    ...
    {
      authentication: [
        buildProfile("http-bearer", { token: "ghp_CANARYTOKEN4b2" }),
      ],
    },
...
test("CON-01-LM: every list limit in DEFINITION_LIMITS is enforced at the boundary", () => {
  const definition = buildDefinition();
  const noneProfile = (index: number) =>
    ...
    [
      ...
      (count) => ({
        capabilities: Array.from({ length: count }, (_, index) =>
          buildCapability({ nativeId: `op${index}`, authentication: [] }),
        ),
      }),
    ],
    [
  ...
  for (const [name, build] of cases) {
    ...
    assert.ok(
      normalizedDefinitionSchema.safeParse({ ...definition, ...build(limit) })
        .success,
      `${name} at ${limit}`,
    );
    rejects(
      normalizedDefinitionSchema,
      { ...definition, ...build(limit + 1) },
      `${name} at ${limit + 1}`,
    );
  }
  const json = JSON.stringify(definition);
  const exact =
    json + " ".repeat(DEFINITION_LIMITS.bytes - Buffer.byteLength(json));
  assert.equal(
    parseNormalizedDefinition(exact).definitionRef,
...
test("CON-01-EX: native extensions are bounded inert JSON and refuse reserved keys at any depth", () => {
  ...
  assert.ok(
    messages(rejected).includes("JSON value exceeds bounds: reserved-key"),
  );
  for (const value of [
  ...
  const nest = (depth: number): unknown =>
    depth === 0 ? 1 : { level: nest(depth - 1) };
  assert.ok(
    nativeExtensionsSchema.safeParse({
      "x-deep": nest(EXTENSION_LIMITS.depth - 2),
    }).success,
  );
  rejects(
    nativeExtensionsSchema,
    { "x-deep": nest(EXTENSION_LIMITS.depth - 1) },
    "depth",
  );
  assert.ok(
    nativeExtensionsSchema.safeParse(
      Object.fromEntries(
        Array.from({ length: EXTENSION_LIMITS.keys }, (_, index) => [
          `x-${index}`,
          index,
        ]),
      ),
  ...
  rejects(
    nativeExtensionsSchema,
    Object.fromEntries(
      Array.from({ length: EXTENSION_LIMITS.keys + 1 }, (_, index) => [
        `x-${index}`,
        index,
      ]),
    ),
  ...
  rejects(
    nativeExtensionsSchema,
    { "x-many": Array.from({ length: EXTENSION_LIMITS.nodes }, () => 0) },
    "nodes",
  );
  rejects(
    nativeExtensionsSchema,
    { "x-long": "x".repeat(EXTENSION_LIMITS.stringLength + 1) },
    "string",
  );
  rejects(
    nativeExtensionsSchema,
    { "x-big": Array.from({ length: 20 }, () => "x".repeat(8000)) },
    "bytes",
  );
  for (const value of [
    { "x-fn": () => 1 },
  ...
  const overBudget = normalizedDefinitionSchema.safeParse({
    ...definition,
    capabilities: capabilities(9),
  });
  assert.equal(overBudget.success, false);
  assert.ok(
    messages(overBudget).includes(
      "Native extensions exceed the definition budget",
    ),
  );
});
