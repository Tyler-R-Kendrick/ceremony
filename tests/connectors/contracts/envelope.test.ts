...
function projectFromManifest(manifest: ConnectorManifest): ConnectorProject {
  ...
  const documents = new Map<
    string,
    Array<{ workflowId: string; summary: string; steps: ReturnType<typeof defaultWorkflowSteps> }>
  >();
  ...
  return connectorProjectSchema.parse({
    ...
    workflows: [...documents].map(([document, workflows]) => ({
      ...
      sourceDescriptions: [{ name: "provider", type: "openapi", url: SOURCE_URL }],
      workflows,
    })),
  });
}

const shippedProjects = [...manifests, ...serviceManifests].map(projectFromManifest);
const authoredProjects = [
  authoredProject(["oauth-code"], { id: "acme", name: "Acme", description: "Acme OAuth." }),
  authoredProject(["api-key", "basic", "form"], { id: "multi", name: "Multi", description: "" }),
  authoredProject([...flowKinds], { id: "every-kind", name: "Every kind", description: "All families." }),
];
...
test("AC-IMP-01: shipped and authored v1 projects upgrade, round-trip and downgrade to identical projects with nothing added", async () => {
  for (const project of [...shippedProjects, ...authoredProjects]) {
    ...
    assert.deepEqual(exportConnectorFiles(downgraded.project!), exportConnectorFiles(project));
    assert.deepEqual(downgraded.diagnostics.map((issue) => issue.code), ["envelope.v1.provenance-dropped"]);
    assert.ok(downgraded.diagnostics.every((issue) => issue.severity === "info"));
    assert.deepEqual(Object.keys(envelope), ["format", "version", "profile", "definition", "sources", "project"]);
    ...
    assert.equal(definition.authentication.length, project.manifest.methods.length);
    ...
    const text = JSON.stringify(envelope);
    for (const forbidden of ["\"binding\"", "\"destinations\"", "\"credential", "\"approved\"", "\"secret\":", "\"token\":"])
...
test("CON-02-02: v1 spellings a profile cannot carry are derived safely while the project stays untouched", async () => {
  const project = authoredProject(["api-key", "device"], { id: "-legacy", name: `Legacy${control(9)}name`, description: "x" });
  project.manifest.methods[0]!.id = "1-legacy";
  project.manifest.methods[0]!.label = `Tab${control(9)}label`;
  project.manifest.methods[0]!.contract.workflows[0]!.workflowId = "1-legacy";
  project.workflows[0]!.workflows[0]!.workflowId = "1-legacy";
  project.manifest.methods[1]!.id = "ceremony-1-legacy";
  project.manifest.methods[1]!.contract.workflows[0]!.workflowId = "ceremony-1-legacy";
  project.workflows[0]!.workflows[1]!.workflowId = "ceremony-1-legacy";
  ...
  if (first!.kind !== "ceremony-method" || second!.kind !== "ceremony-method") throw new Error("unreachable");
  ...
  assert.equal(envelope.project!.manifest.methods[0]!.label, `Tab${control(9)}label`);
});
...
test("AC-IMP-02: a public API is a valid description with an explicit none profile and no fabricated login", () => {
  ...
  const envelope = buildEnvelope({ definition: portableDefinition(definition) });
  assert.deepEqual(envelope.definition.authentication.map((profile) => profile.kind), ["none"]);
  assert.deepEqual(envelope.definition.capabilities[0]!.authentication, []);
  assert.equal(envelope.project, undefined);
  const text = JSON.stringify(envelope);
  for (const forbidden of ["\"methods\"", "\"fields\"", "api-key", "password", "\"token\""])
    assert.equal(text.includes(forbidden), false, forbidden);
  const parsed = parseConnectorEnvelope(text);
  assert.equal(parsed.version, 2);
  ...
  assert.equal(connectorEnvelopeSchema.safeParse({ ...envelope, project: authoredProjects[0]!.manifest }).success, false);
});

test("CON-02-04: envelope headers, cross references, hostile keys and the byte ceiling are enforced", async () => {
  const envelope = await upgradeConnectorProject(authoredProjects[0], producer, AT);
  const project = envelope.project!;
  const ceremonyProfile = envelope.definition.authentication[0]!;
  const withProject = (definition: Partial<typeof envelope.definition>, keepProject = true) =>
    connectorEnvelopeSchema.safeParse({ ...envelope, definition: { ...envelope.definition, ...definition }, ...(keepProject ? {} : { project: undefined }) }).success;
  assert.equal(withProject({}, true), true);
  assert.equal(withProject({}, false), false, "ceremony-method profile without a project");
  assert.equal(withProject({ authentication: [buildProfile("none")] }), false, "project without ceremony-method profile");
  assert.equal(withProject({ authentication: [{ ...ceremonyProfile, methodId: "missing" } as typeof ceremonyProfile] }), false, "unknown method");
  assert.equal(withProject({ authentication: [{ ...ceremonyProfile, flowKind: "device" } as typeof ceremonyProfile] }), false, "flow kind mismatch");
  assert.equal(connectorEnvelopeSchema.safeParse({ ...envelope, profile: { id: "ceremony-connector/3", producer } }).success, false);
  assert.equal(connectorEnvelopeSchema.safeParse({ ...envelope, project: { ...project, extra: true } }).success, false);
  for (const text of [
    ...
    "null",
    "\"ceremony-connector\"",
    JSON.stringify(envelope).replace("{\"format\"", "{\"__proto__\":{\"polluted\":true},\"format\""),
  ])
    assert.throws(() => parseConnectorEnvelope(text), text.slice(0, 40));
  assert.equal(({} as Record<string, unknown>).polluted, undefined);
  const json = JSON.stringify(envelope);
  const exact = json + " ".repeat(ENVELOPE_LIMITS.bytes - Buffer.byteLength(json));
  assert.equal(parseConnectorEnvelope(exact).version, 2);
  assert.throws(() => parseConnectorEnvelope(exact + " "), /import limit/);
  assert.throws(() => parseConnectorEnvelope(json.replace("Acme OAuth.", "é".repeat(2 * 1024 * 1024))), /import limit/);
...
test("AC-IMP-14: a partially supported description round-trips its project and names every loss explicitly", async () => {
  const upgraded = await upgradeConnectorProject(authoredProjects[1], producer, AT);
  const blocking = buildCompatibilityIssue();
  const definition = {
    ...upgraded.definition,
    identity: { ...upgraded.definition.identity, ecosystem: "openapi", nativeId: "example/multi" },
    authentication: [...upgraded.definition.authentication, buildProfile("oauth-authorization-code")],
    configuration: [...upgraded.definition.configuration, { name: "EXTRA_SETTING", source: "host" as const, classification: "public" as const, required: false }],
    capabilities: [buildCapability()],
    events: [{ nativeId: "pet.created", transport: "http-webhook" as const, verification: "standard-webhooks" as const }],
    declaredServers: [...upgraded.definition.declaredServers, { url: "https://api.example.invalid/v2", status: "declared" as const }],
    compatibility: { issues: [blocking], dimensions: completeDimensions({ import: "exact", authorize: "unsupported" }) },
    nativeExtensions: { "x-vendor": { note: "kept inert" } },
  };
  const envelope = connectorEnvelopeSchema.parse({
    ...upgraded,
    definition: { ...definition, normalizedDigest: await normalizedDigestOf(definition) },
  ...
  assert.ok(downgraded.diagnostics.every((issue) => issue.executionImpact === "none" && issue.dimension === "export"));
  assert.match(downgraded.diagnostics[0]!.message, /1 other authentication profile/);
  assert.equal(parsed.envelope.definition.compatibility.issues[0]!.severity, "blocking");
  assert.equal(parsed.envelope.definition.compatibility.dimensions.authorize, "unsupported");
...
test("CON-02-06: digests, dimensions and kinds are stable helpers", async () => {
  ...
  assert.equal(digest, await normalizedDigestOf(portableDefinition(definition)));
  assert.equal(digest, await normalizedDigestOf({ ...definition, definitionRef: "definition:other", normalizedDigest: "0".repeat(64) }));
  assert.equal(digest, await normalizedDigestOf(Object.fromEntries(Object.entries(definition).reverse())));
  assert.notEqual(digest, await normalizedDigestOf({ ...definition, capabilities: [] }));
  assert.equal(await verifyNormalizedDigest(definition), false);
  assert.equal(await verifyNormalizedDigest({ ...definition, normalizedDigest: digest }), true);
  assert.deepEqual(Object.values(completeDimensions({})), Array(12).fill("unsupported"));
  assert.equal(completeDimensions({ import: "exact" }).import, "exact");
  assert.equal(authenticationKinds.length, 13);
  assert.ok(authenticationKinds.includes("none") && authenticationKinds.includes("ceremony-method") && authenticationKinds.includes("unsupported"));
});
