...
import {
  assertMatchesPinnedServerDetail,
  loadServer,
  mcpBinding,
} from "./support.js";
...
test("exports a schema-valid server.json describing only the served hosted endpoint", async () => {
  ...
  assert.deepEqual(result.document.remotes, [
    { type: "streamable-http", url: "https://mcp.example.com/mcp" },
  ]);
  assert.deepEqual(result.endpoints, ["https://mcp.example.com/mcp"]);
  assert.ok(
    !("packages" in result.document),
    "packages never run here are never exported",
  );
  assert.ok(
    result.losses.some(
  ...
  const parsed = JSON.parse(new TextDecoder().decode(result.bytes)) as Record<
    string,
    unknown
  >;
  ...
  const meta = (parsed["_meta"] as Record<string, Record<string, unknown>>)[
    "io.modelcontextprotocol.registry/publisher-provided"
  ]!;
  assert.deepEqual(meta["ceremony"], {
    exporter: "mcp-registry-server-json/1.0.0",
    normalizedDigest: def.normalizedDigest,
    bindingRevision: 3,
  ...
  assert.doesNotMatch(
    text,
    /X-API-Key|anonymous\.modelcontextprotocol\.io|binding:|tenant-a|src:mcp-registry/,
    "no imported remotes, binding refs, tenant or source references",
  );
  const withHeaders = exportServerJson({
    ...
    publication: {
      ...
      version: "2.0.0-rc.1",
      title: "Ceremony MCP",
      description: "Hosted Ceremony MCP endpoint",
      headers: [
        { name: "X-Ceremony-Tenant", isRequired: true, isSecret: false },
      ],
    },
  });
  assert.equal(withHeaders.document.version, "2.0.0-rc.1");
  assert.deepEqual(withHeaders.document.remotes[0]!.headers, [
    { name: "X-Ceremony-Tenant", isRequired: true, isSecret: false },
  ]);
});
...
test("refuses browser-only connectors, non-MCP bindings, unserved or unverified endpoints and unapproved bindings", async () => {
  const def = await definition();
  const attempt =
    (input: Partial<Parameters<typeof exportServerJson>[0]>) => () =>
      exportServerJson({
        definition: def,
        binding: mcpBinding(def.definitionRef),
        implementationEvidence: evidence,
  ...
  assert.throws(
    attempt({ binding: mcpBinding(def.definitionRef, { runtime: "browser" }) }),
  ...
  assert.throws(
    attempt({
      implementationEvidence: {
        servedEndpoints: [
          { ...evidence.servedEndpoints[0]!, runtime: "browser" },
        ],
      },
    }),
    refused("export.browser-only"),
  ...
  assert.throws(
    attempt({ binding: mcpBinding(def.definitionRef, { httpOnly: true }) }),
    refused("export.binding-not-mcp"),
  );
  assert.throws(
    attempt({
      implementationEvidence: {
  ...
  assert.throws(
    attempt({ implementationEvidence: { servedEndpoints: [] } }),
    refused("export.endpoint-not-served"),
  );
  assert.throws(
    attempt({
      implementationEvidence: {
        servedEndpoints: [
          { ...evidence.servedEndpoints[0]!, evidence: "unit" },
        ],
      },
    }),
  ...
  assert.throws(
    attempt({
      binding: mcpBinding(def.definitionRef, { status: "suspended" }),
    }),
    refused("export.binding-not-approved"),
  );
  assert.throws(
    attempt({ binding: mcpBinding("def:mcp-registry:other") }),
    refused("export.binding-definition-mismatch"),
  );
  assert.throws(
    attempt({ publication: { ...publication, authorized: false } }),
    refused("export.publication-unauthorized"),
  );
  assert.throws(
    attempt({ publication: { ...publication, version: "^1.0.0" } }),
  );
  assert.throws(
    attempt({ publication: { ...publication, name: "no-namespace" } }),
  );
  assert.throws(
    attempt({
      binding: mcpBinding(def.definitionRef, { pathPrefix: "/other" }),
    }),
    refused("export.endpoint-not-served"),
  );
  const prefixed = exportServerJson({
    definition: def,
    binding: mcpBinding(def.definitionRef, { pathPrefix: "/mcp" }),
    implementationEvidence: evidence,
    publication,
  });
  assert.equal(
    prefixed.document.remotes[0]!.url,
...
test("a private or loopback endpoint may only be described for a fixture or approved-private target", async () => {
  ...
  const binding = mcpBinding(def.definitionRef, {
  ...
  assert.throws(
    () =>
      exportServerJson({
        definition: def,
        binding,
        implementationEvidence: loopback,
        publication,
      }),
    (error: unknown) =>
      error instanceof ConnectorError &&
      error.detail === "export.private-endpoint",
  );
  const fixture = exportServerJson({
    definition: def,
    binding,
    implementationEvidence: loopback,
    publication: {
      ...publication,
      target: { sourceId: "registry-fixture", network: "loopback-fixture" },
    },
  });
  ...
  const intranetBinding = mcpBinding(def.definitionRef, {
    origin: "https://mcp.corp.internal",
  ...
  assert.throws(
    () =>
      exportServerJson({
        ...
        implementationEvidence: intranet,
        publication,
      }),
    (error: unknown) =>
      error instanceof ConnectorError &&
  ...
  const approved = exportServerJson({
    definition: def,
    binding: intranetBinding,
    implementationEvidence: intranet,
    publication: {
      ...publication,
      target: { sourceId: "corp-registry", network: "approved-private" },
    },
  });
...
test("publishes only to the local double, with a privately read token and a journaled effect", async () => {
  const def = await definition();
  const exported = exportServerJson({
    definition: def,
    binding: mcpBinding(def.definitionRef),
    implementationEvidence: evidence,
    publication,
  ...
  const double = await startMcpRegistryDouble({
    publishTokens: ["local-publish-token-3"],
  });
  const ports = memoryPorts();
  ports.configuration.set(
    "MCP_REGISTRY_PUBLISH_TOKEN",
    "local-publish-token-3",
  );
  try {
    ...
    await assert.rejects(
      ...
      (error: unknown) =>
        error instanceof ConnectorError &&
        error.detail === "export.publication-unauthorized",
    );
    await assert.rejects(
      publishServerJson({
        client,
    ...
    const published = await publishServerJson({
      client,
      document: exported.document,
      publication: { authorized: true },
      effects: ports.effects,
    ...
    await assert.rejects(
      ...
      (error: unknown) =>
        error instanceof ConnectorError &&
        error.detail === "export.publish.repeated",
    );
    assert.equal(
      double.received("POST", "/v0.1/publish").length,
      1,
      "a repeated effect does not publish twice",
    );
    let officialCalls = 0;
    ...
    await assert.rejects(
      publishServerJson({
        client,
        document: { ...exported.document, version: "9.9.9" },
        publication: { authorized: true },
      }),
      (error: unknown) =>
        error instanceof ConnectorError &&
