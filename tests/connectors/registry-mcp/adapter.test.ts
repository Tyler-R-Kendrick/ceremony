...
function context(
  binding: AdapterCallContext["binding"],
  ports = memoryPorts(),
): AdapterCallContext {
  return {
    actor: fixtureActor,
...
test("reports catalog-only support with execution explicitly unsupported", () => {
  ...
  for (const dimension of ["discover", "import", "export"] as const)
    assert.equal(byDimension[dimension]!.implementation, "implemented");
  for (const dimension of [
    ...
    assert.equal(
      byDimension[dimension]!.implementation,
      "unsupported",
      dimension,
    );
    assert.equal(byDimension[dimension]!.evidence, "not-tested");
  }
  assert.deepEqual(byDimension["invoke"]!.limitations, [
  ...
  assert.equal(
    adapter
      .capabilities(new Set(["MCP_REGISTRY_TOKEN"]))
      .find((row) => row.dimension === "discover")!.configuration,
    "ready",
  );
  ...
  assert.ok(
    entry.capabilities.every(
      (row) =>
        !["live-authorized", "deployed-authorized"].includes(row.evidence),
    ),
  );
  ...
  assert.deepEqual(OFFICIAL_MCP_REGISTRY_DESTINATION, {
...
test("discovers live from the binding's approved destination and refuses any other", async () => {
  ...
  try {
    ...
    const binding = registryBinding(double.origin, {
      settings: { registrySource: { destinationId: "registry-fixture" } },
    });
    const result = await adapter.discover!(context(binding), {
      limit: 3,
      query: "io.github",
    });
    ...
    assert.deepEqual(
      ...
      ["io.github.a/b", "io.github.a/b", "io.github.c/d"],
    );
    ...
    const more = await adapter.discover!(context(binding), {
      limit: 3,
      query: "io.github",
      cursor: result.nextCursor,
    });
    assert.deepEqual(
      more.items.map((item) => item.identity.nativeId),
      ["io.github.e/f"],
    );
    await assert.rejects(
      adapter.discover!(
        context(
          registryBinding(double.origin, {
            settings: { registrySource: { destinationId: "somewhere-else" } },
          }),
        ),
        {},
      ),
      (error: unknown) =>
        ...
        error.detail === "registry.destination-unapproved",
    );
    await assert.rejects(
      adapter.discover!(
        context(
    ...
    await assert.rejects(
      adapter.discover!(context(binding), { limit: 101 }),
      (error: unknown) =>
        error instanceof ConnectorError &&
        error.detail === "registry.limit.invalid",
    );
    assert.equal(
      registrySourceFromBinding(registryBinding(double.origin)).baseUrl,
      double.origin,
      "a single destination needs no settings",
    );
  } finally {
    await double.close();
  }
...
test("serves snapshots when a store is configured, refreshing only on request, and marks tombstones", async () => {
  ...
  try {
    ...
    assert.equal(
      live.freshness.source,
      "live",
      "no snapshot yet falls back to live",
    );
    const refreshed = await adapter.discover!(context(binding), {
      refresh: true,
      limit: 10,
    });
    ...
    assert.equal(
      double.received("GET", "/v0.1/servers").length,
      requestsAfterRefresh,
      "no request without refresh",
    );
    assert.equal(cached.items.length, 3);
    assert.ok(cached.nextCursor);
    const rest = await adapter.discover!(context(binding), {
      limit: 10,
      cursor: cached.nextCursor,
    });
    assert.equal(rest.items.length, 4);
    const again = await adapter.discover!(context(binding), {
      refresh: true,
      limit: 10,
    });
    const alpha = again.items.find(
      (item) => item.identity.nativeId === "com.example/alpha",
    )!;
    assert.equal(alpha.status, "deleted");
    assert.equal(alpha.provenance?.tombstone, "deleted");
    const searched = await adapter.discover!(context(binding), {
      query: "HYBRID",
      limit: 10,
    });
    assert.deepEqual(
    ...
    assert.equal(
      await snapshots.read("tenant-b", "registry-fixture"),
      undefined,
    );
  } finally {
    await double.close();
...
test("imports bytes with exact-byte provenance and unwraps registry responses", async () => {
  ...
  const outcome = await adapter.import!(context(binding), {
    bytes,
  ...
  assert.equal(
    ...
    `src:mcp-registry:${outcome.source.digest.value}`,
  );
  assert.equal(outcome.definitions[0]!.sourceRef, outcome.source.sourceRef);
  assert.equal(
    outcome.definitions[0]!.identity.nativeId,
    "io.modelcontextprotocol.anonymous/hybrid-mcp",
  );
  ...
  assert.deepEqual(
    (
      outcome.definitions[0]!.nativeExtensions[
        "io.ceremony.connectors/source"
      ] as { sourceId: string }
    ).sourceId,
    "registry-fixture",
  );
  const wrapped = registryEntry(loadServer("brave-search"), {
    status: "deprecated",
    statusMessage: "old",
  ...
  assert.equal(
    (
      wrappedOutcome.definitions[0]!.nativeExtensions[
        MCP_REGISTRY_OFFICIAL_META_KEY
      ] as { status: string }
    ).status,
    "deprecated",
  ...
  assert.ok(
    wrappedOutcome.issues.some((issue) => issue.code === "version.deprecated"),
  );
  await assert.rejects(
    adapter.import!(context(binding), {
    ...
    (error: unknown) =>
      error instanceof ConnectorError &&
      error.detail === "server-json.oversized",
  );
  const viaEntry = await importRegistryEntry(
    {
      server: wrapped.server as never,
      official: wrapped._meta[MCP_REGISTRY_OFFICIAL_META_KEY],
    },
    {
      sourceRef: "src:x",
...
test("exports only through host-approved settings naming the MCP binding and evidence", async () => {
  ...
  const imported = await adapter.import!(
    context(registryBinding("https://registry.example.com")),
    {
      bytes: loadServerBytes("hybrid"),
      mediaType: "application/json",
      origin: { kind: "upload" },
    },
  );
  ...
  await assert.rejects(
    ...
    (error: unknown) =>
      error instanceof ConnectorError &&
      error.detail === "export.requires-mcp-binding",
  );
  await assert.rejects(
    adapter.export!(
      context(
        registryBinding("https://registry.example.com", {
    ...
    (error: unknown) =>
      error instanceof ConnectorError && error.detail === "export.format",
  );
  await assert.rejects(
    adapter.export!(
      context(
    ...
    (error: unknown) =>
      error instanceof ConnectorError &&
      error.detail === "export.tenant-mismatch",
  );
  await assert.rejects(
    adapter.export!(
      context(
    ...
    (error: unknown) =>
      error instanceof ConnectorError && error.detail === "export.browser-only",
  );
  const outcome = await adapter.export!(
    ...
    { ...request, includeNativeExtensions: true },
  );
  ...
  assert.ok(
    outcome.losses.some(
      (loss) => loss.code === "structure.native-extensions-omitted",
    ),
  );
  assert.ok(
    outcome.losses.some(
      (loss) => loss.code === "executable-code.package-omitted",
    ),
  );
});
...
test("a private source reads its bearer token through the configuration port per call", async () => {
  ...
  try {
    ...
    const binding = registryBinding(double.origin, {
      settings: {
        registrySource: {
          destinationId: "registry-fixture",
          authorization: {
            kind: "bearer",
            configurationName: "MCP_REGISTRY_TOKEN_PRIVATE",
          },
        },
      },
    });
    ...
    await assert.rejects(
      adapter.discover!(context(binding, ports), {}),
      (error: unknown) =>
        error instanceof ConnectorError &&
        error.detail === "registry.unauthorized",
    );
    ports.configuration.set("MCP_REGISTRY_TOKEN_PRIVATE", token);
    const result = await adapter.discover!(context(binding, ports), {
      limit: 2,
    });
    ...
    await assert.rejects(
      ...
      (error: unknown) =>
        error instanceof ConnectorError &&
        error.detail === "registry.settings.invalid",
    );
  } finally {
    await double.close();
  }
