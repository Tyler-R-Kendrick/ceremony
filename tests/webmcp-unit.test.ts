...
test("AC-15 AC-16 AC-22: local and native tools enforce current public fields without private references", async () => {
  ...
  const local = createCeremonyTools(
    "safe",
    manifest,
    invoke,
    signal,
  ...
  assert.deepEqual(
    local.map(({ execute, ...definition }) => definition),
    native.map(({ execute, ...definition }) => definition),
  );
  for (const tools of [local, native]) {
    const submit = tools.find((t) => t.name === "safe_submit")!;
    for (const input of [
      { secretRef: crypto.randomUUID() },
      { values: { email: "not-current" } },
      { values: { note: "unknown-sensitive" } },
      { values: { choice: "yes" }, source: "ui" },
    ]) {
      const before = calls;
      assert.equal(
        Reflect.get(Object(await submit.execute(input)), "ok"),
        false,
      );
    ...
    assert.equal(
      Reflect.get(
        Object(await submit.execute({ values: { choice: "yes" } })),
        "ok",
      ),
      true,
    );
