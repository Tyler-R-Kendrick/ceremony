...
/*
 * Bounded reading of untrusted JSON. A control-plane response carries a
 * customer's own target configuration — an inline OpenAPI document, a Lambda
 * tool schema — so it is imported data, not a trusted answer: it is size-bound
...
export const DEFAULT_AGENTCORE_JSON_BOUNDS: JsonBounds = Object.freeze({
...
export function parseBoundedJsonText(
  text: string,
  bounds: JsonBounds = DEFAULT_AGENTCORE_JSON_BOUNDS,
): unknown {
