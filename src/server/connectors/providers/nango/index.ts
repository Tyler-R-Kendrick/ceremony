...
import { inspectNangoConnection, type NangoConnectionInspection } from "./inspect.js";
...
export { NANGO_SNAPSHOT_FORMAT, nangoSnapshotSchema, serviceSlug } from "./discover.js";
export {
  NANGO_CONFIGURATION,
  NANGO_CONFIGURATION_NAMES,
  NANGO_DEFAULT_API_ORIGIN,
...
export interface NangoAdapter extends ConnectorAdapter {
  ...
  inspectConnection(ctx: AdapterCallContext): Promise<NangoConnectionInspection>;
  /** Applies a verified webhook to the connection after checking Nango's current state. */
  reconcileEvent(ctx: AdapterCallContext, event: VerifiedEventEnvelope): Promise<NangoReconcileResult>;
  ...
  catalogEntries(ctx: AdapterCallContext, present: ReadonlySet<string>): Promise<CatalogEntry[]>;
}

/** Approved destinations for a Nango binding; a host builds its binding from these, never from a request. */
export function nangoDestinations(options: {
  apiOrigin?: string;
  ...
  return [
    { id: "api", origin: options.apiOrigin ?? NANGO_DEFAULT_API_ORIGIN, network },
    { id: "connect", origin: options.connectOrigin ?? NANGO_DEFAULT_CONNECT_ORIGIN, network },
  ];
}

export function createNangoAdapter(options: NangoAdapterOptions = {}): NangoAdapter {
  ...
  const identity = { adapterVersion: NANGO_ADAPTER_VERSION, runtime: "hosted-server" as const };
  const adapter: NangoAdapter = {
    ...
    capabilities(present) {
      const core = present.has(NANGO_CONFIGURATION_NAMES.secretKey) && present.has(NANGO_CONFIGURATION_NAMES.environment);
      const configuration = core ? ("ready" as const) : ("missing" as const);
      const row = (
        dimension: CapabilityStatus["dimension"],
        input: Partial<Pick<CapabilityStatus, "implementation" | "configuration" | "limitations" | "evidence">> = {},
      ) =>
      ...
      return [
        row("discover", {
          limitations: ["GET /integrations is not paginated upstream; function listing is paginated per integration."],
        }),
        row("import", {
          configuration: "not-applicable",
          limitations: ["Imports API snapshots of integration + deployed functions; nango.yaml is refused as legacy."],
        }),
        row("configure", { configuration: "not-applicable", evidence: "unit" }),
        row("authorize", {
          limitations: ["Connect UI requires a person; interruption policy 'none' yields human-required."],
        }),
        row("verify", {
          limitations: [
        ...
        row("invoke", {
          limitations: ["Proxy and action calls use only bound operations; asynchronous actions (X-Async) are not used."],
        }),
        row("events", {
          configuration: present.has(NANGO_CONFIGURATION_NAMES.webhookSigningKey) && core ? "ready" : "missing",
          limitations: [
        ...
        row("reconnect", {
          limitations: ["Only connections created through a connect session can be reconnected (documented Nango restriction)."],
        }),
        row("disconnect", {
          limitations: ["Default scope is local unlink; DELETE /connections/{id} only under scope 'broker' and without unapproved shared references."],
        }),
        row("revoke", {
          implementation: "unsupported",
          configuration: "not-applicable",
          limitations: ["Nango documents no provider-grant revocation endpoint; broker deletion is not upstream revocation."],
        }),
        row("export", {
          implementation: "unsupported",
          configuration: "not-applicable",
          limitations: ["No Nango-native export format: nango.yaml is deprecated and Zero YAML is executable TypeScript."],
        }),
        row("delegate", {
          limitations: [
    ...
    reconnect: (ctx, intent) => authorizeNango(runtime, ctx, intent, "reconnect"),
    ...
    reconcileEvent: (ctx, event) => reconcileNangoEvent(runtime, inbox, ctx, event),
    deleteBrokerConnection: (ctx, options) => deleteNangoBrokerConnection(runtime, ctx, options),
    captureIntegration: (ctx, uniqueKey) => captureNangoIntegration(runtime, ctx, uniqueKey),
    catalogEntries: (ctx, present) => nangoCatalogEntries(adapter, runtime, ctx, present),
  };
  return adapter;
}
