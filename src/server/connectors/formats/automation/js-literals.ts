 ...
 *
 * A key the source wrote is a key a stranger chose, and `out[key] = value` for
 * `__proto__` does not add a property at all — it calls the prototype setter,
 * so the object this returns would no longer be a plain object. Everything
 * downstream refuses such an object rather than copying it: `inertCopy` returns
 * undefined for it, `measureJsonValue` calls it `not-json`, and the schema then
 * rejects the whole description. One reserved key would therefore cost the
 * reader its entire answer, so the reserved keys are dropped here, exactly as
 * `fromJsonValue` drops them on the way in.
 */
