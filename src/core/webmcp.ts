...
export function createCeremonyTools(
  ...
  return (
    ["start", "read", ...actionNames, "navigate", "request-input"] as const
  ).map((action): CeremonyTool => ({
    ...
    execute: async (input, options = { signal }) => {
      try {
        ...
        if ("secretRef" in command) throw new Error("Unexpected reference");
        if (action === "submit" && "values" in command) {
...
export async function registerCeremonyTools(
  ...
  for (const tool of createCeremonyTools(
    prefix,
    manifest,
    invoke,
    signal,
