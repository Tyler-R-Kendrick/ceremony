...
- <a id="issued-values"></a>**An issued value is kept only where the plan says.** A plan may declare `issued`: for each kind (`client-id`, `client-secret`), the exact label of the read-only field that displays it, and a `keep` sink. On every observation on an allowed origin the driver looks for exactly one input with that label and reads it through the page adapter. The adapter reads only a visible, read-only control that the observation still describes, so a field the driver typed into cannot be read back out. All declared values are read from one page or none is, so a client ID and a secret from different pages are never paired; `keep` then receives them once. In a run, that sink writes them straight into a run-bound `common.oauth-client` record. The interpreter cannot name, point to or learn of a read. The values are in no snapshot, history, note, transcript or result; the transcript records only a `kept` step that names the kinds. A secret is guarded from the moment it is read, so a page that also prints it fails with `CeremonySecretLeak`. A label that matches two fields reads neither, and a `done` claim is refused while a declared value is unread. Without `issued`, the rule above stands: a displayed credential is for a person.
