...
| `registration-enrolls-an-authenticator`                   | `account-registration` | OTP / magic link / MFA           | `account-absent`, `address-unused`, `mailbox-readable`                      | `email`, `password`, `password-confirm`, `verification-code`                    | completed; the seed kept is the one the account enrolled          |
...
| `registration-requiring-terms`                            | `account-registration` | Forms/session auth               | `account-absent`, `address-unused`, `mailbox-readable`                      | `email`, `password`, `password-confirm`, `verification-code`                    | completed                                                         |
...
| `device-authorization-with-consent`                       | `device`               | OAuth device authorization       | `account-exists`, `account-verified`                                        | `username`, `password`, `user-code`                                             | completed; the device's poll gets one token, for this account     |
...
Two walls are about what the caller was given rather than what the page is. A **device authorization** page (RFC 8628) asks for the user code shown on a device: `device-approval` was given it as a `user-code` role and types it; the other two were not, so a person holding the device enters it at the verification URI, or with nobody to ask the attempt stops as `device-code-required`. The device code itself never reaches a page, a role or a snapshot. `device-authorization-with-consent` runs the whole RFC 8628 exchange: the device asks at `/device_authorization`, is told `authorization_pending` while it polls `/token`, the code entered at the verification URI (no query) leads to a consent screen naming the device's client, and only after Allow does the next poll return one access token, for the approving account; the device code is spent after that. A **required choice** - the country picker `requireRegion` adds to registration - is answered by a plan's `choices` entry for that field's label, by a person when the plan made none, and otherwise stops as `choice-required` before anything is submitted. Neither is ever guessed: the first option is rarely the person's country, and no other code is the device's.

**Enrolling a second factor** is the one place a provider hands the caller a secret to keep. `enrollTotp` sends an account with no authenticator to "Set up two-factor authentication" after its address is confirmed, or at its next sign-in. The page shows a fresh RFC 6238 seed as a setup key in a read-only field and turns the factor on only for a code from it. `registration-enrolls-an-authenticator` declares that field as an issued `totp-seed`: the driver reads it, guards every spelling of it, hands it to the plan's custody sink, and answers the page with a `totp-code` it computes from the seed at fill time. The caller supplies no authenticator code, and none reaches the interpreter. From then on that account's sign-in asks for a code from its own seed.
