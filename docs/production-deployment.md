...
This server path is implemented and locally tested; the connected native owner/requester UI and A2H delivery are not yet mounted. Do not advertise a complete end-user assignment experience from these JSON routes. Requests and shared app eligibility expire after 24 hours; rotation of an existing shared app requires a new host configuration version. Shared configuration is bound to tenant, site, callback origin, environment, configuration version, designated owner and host-selected scopes. It is not reused over partial session credentials. Raw app values remain encrypted, and user tokens/consent remain session-specific. Expired-record retention and user-facing assignment recovery remain release work, not a production certification claim.
