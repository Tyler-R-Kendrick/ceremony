...
| Name                    | Purpose                                                                                                                                                                                                             |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `CEREMONY_TENANT_CLAIM` | Claim whose value is the tenant (for example an organization id): a top-level name, or a JSON Pointer into nested claims (see below). A token without it, or with a malformed or server-reserved value, is refused. |
| `CEREMONY_ROLES_CLAIM`  | Claim carrying roles, as a string array or a space-separated string: a top-level name or a JSON Pointer. Default `ceremony_roles`.                                                                                  |
...
Both claim settings accept two spellings, told apart by the first character. A value that does not start with `/` is one top-level claim name, taken whole, so a URL-namespaced claim such as `https://example.invalid/claims.tenant` works as written; dots are never separators. A value that starts with `/` is an [RFC 6901](https://www.rfc-editor.org/rfc/rfc6901) JSON Pointer into nested claims: `/realm_access/roles` reads `roles` inside the `realm_access` object, and a slash inside a key is escaped as `~1` (`/https:~1~1example.invalid~1claims/org`), a tilde as `~0`. A pointer walks objects only, at most eight levels: an array, string, number or null on the way is refused rather than indexed or coerced. The tenant must be a string (a numeric claim is refused, not converted), and roles a string array or a space-separated string. A pointer with an empty segment, a trailing `/`, another `~` escape or more than eight segments fails configuration at start-up, before any token is read. For roles, a pointer whose objects simply lack the key counts as no roles claim (executor only); for the tenant, it is refused like any missing tenant.
