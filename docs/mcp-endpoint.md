...
- `connector_catalog`, `connector_status`, `connector_connect` and `connector_invoke` — only when the host passes the optional `connectors` option to `createCeremonyMcpHandler`. They are a transport over the connector command service, which re-checks authorization itself; the tools enforce projection and output policy. `connector_connect` never returns a link or a code, `connector_status` never returns credentials or links, and `connector_invoke` returns output only when the binding classified that output public — a personal or secret result is withheld from this transport and stays readable by the person in the application.
