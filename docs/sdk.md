...
- `tests/python-client/openapi-python-client.test.ts` generates the Python package the same way and fails if the generator reports any warning. Then [`smoke.py`](../tests/consumers/openapi-python/smoke.py) drives the same handler through it over loopback HTTP, covering the same ground except invocation. It needs the pinned generator ([`requirements.txt`](../tests/consumers/openapi-python/requirements.txt)), so the default test run leaves it out, and run on its own (`npm run test:clients`) it skips without the generator. The `python-client` job in `verify.yml` installs the generator and sets `CEREMONY_REQUIRE_PYTHON_CLIENT=1`, so there the skip becomes a failure.
