...
| ~~TARGET-FRAME~~, TARGET-POPUP, ~~TARGET-AMBIG~~, TARGET-CLOSED                          | **TARGET-FRAME and TARGET-AMBIG covered.** A credential form served by a second origin inside an iframe is driven on all three engines, and the two ways frame selection can fail to identify a document - none at the declared origin, more than one - refuse by name rather than falling back to the page. TARGET-POPUP still needs `popupBinding`, which `browser-executor.ts` deliberately does not do: it aborts a popup and closes the context. TARGET-CLOSED needs a case for the page going away under an attempt. |
