...
Whole ceremonies and fragments use one recipe schema. Publication pins reviewed child versions; import creates a draft. Sharing a procedure never transfers credentials, browser sessions, account ownership or earlier consent. Artifacts must match the new principal, provider/profile, environment, callback origin, target and configuration/permission version before reuse. A run admits only operations for its own provider/profile, plus a closed set of exceptions (`ProtectedCommandService.admits`). Those exceptions are provider-neutral steps registered through `registerNeutral`, which use only neutral vocabulary and opaque run-bound handles; authored steps under an authored profile; and authored account registration ahead of a GitHub App installation. A recipe can still span providers: an invocation that names another `connector` gives every step beneath it that connector's context, resolved by the host for the actor. Each such step is admitted and authorized against its own context rather than the run's, every connector is authorized before the run exists, and a value crosses between contexts only when its vocabulary entry is declared `crossProvider`. Saving a draft that spans providers places each unnamed node under the one approved connector for its provider, and reports a choice between several instead of making it. See [composition](ceremony-teaching.md#reuse-and-composition).
...
- OpenUI owns presentation; registered operations own executable semantics. Existing Arazzo exports are compatibility surfaces, not generated-code authority. An imported Arazzo workflow executes only by compiling to a recipe draft over already-registered operations ([Arazzo profiles](specifications/arazzo-profiles.md)).
