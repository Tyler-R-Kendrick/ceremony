...
| Browser plans                            | **Closer to the driver.** Production login plans can keep provider-issued values (an OAuth client ID and secret) into a host-registered sink (`oauth-client` or `credential-custody`), never a caller or model callback; the declaration is in the plan digest and in the recording a person reviews. The driver chooses `<select>` options by visible label, and never guesses a required choice the plan did not make. RFC 8628 device verification pages are recognised: the user code is typed only when the plan holds it, otherwise a person gets the verification URL without its query.       |
