...
| `action`       | `fill` a `target` with a credential **role** (`username`, `password`, `totp-code`, …), `click` a `target` expecting `navigation` or `same-page`, `check` a checkbox (with the `consent` kinds the tick accepts, below), `select` an `option` of a select by its visible label, or `wait-for` a page mid-transition. There is no value field, so nothing can put one there. |
...
- A tick that accepts a provider's terms, privacy policy or an age attestation records those kinds in `consent`, read from the box the same way the driver reads it, so a reviewer approves the legal act by name. The schema refuses a `check` whose box names a kind its `consent` leaves out, and refuses a tick on a marketing or newsletter opt-in outright.
