...
`tests/contracts/services.test.ts` additionally exercises the official Stripe and Supabase SDKs: exact balance GET/key authorization and project password-grant POST, supported credential rejection, owner-bound resume, inline project-setting reuse and unsafe-origin rejection. These are actual provider-boundary consumer contracts, unlike the generic configured-backend examples. Browser completion tests run those same SDK adapters, private collection and UI with synthetic provider responses. Neither kind of test certifies a credentialed live account.
