...
`tests/contracts/services.test.ts` additionally exercises the official Stripe and Supabase SDKs: exact balance GET/key authorization, project password-grant POST followed by authenticated user GET, supported credential rejection, owner-bound resume, inline project-setting reuse and unsafe-origin rejection. A revoked token or a user lookup that disagrees with the issued session cannot persist access or project settings. These are actual provider-boundary consumer contracts, unlike the generic configured-backend examples. Browser completion tests run those same SDK adapters, private collection and UI with synthetic provider responses. Neither kind of test certifies a credentialed live account.
