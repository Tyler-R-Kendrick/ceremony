...
| [Connector interoperability](connector-interoperability.md) | Source provenance, normalized definitions, the v2 envelope, evidence, lifecycle and projections                                                         | Server-approved runtime bindings and verified provider evidence          |
| [Connector dialects](connector-dialects.md)                 | Supported document dialects and protocol revisions, executable subsets, custody and lifecycle models, import loss, backward compatibility and migration | Version-dispatched readers and host-approved bindings                    |
| [Connector support matrix](connector-support-matrix.md)     | Per-adapter, per-dimension support, required configuration and native limitations                                                                       | The registered adapters themselves; generated, never hand-written        |
...
The complete schema set is connector manifest, method contract, human handoff, recipe, operation, command, demonstration event, method selection, the bounded Arazzo profile, and the connector interoperability documents: connector source v1, connector definition v1, connector envelope v2, capability status v1, verification claim v1, compatibility issue v1, catalog entry v1, connection summary v1 and binding reference v1. Their URN identifiers identify documents; they are not network fetch instructions.
