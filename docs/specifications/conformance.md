...
| CON-01  | Connector identity, definition and catalog contracts reject unsafe identifiers, unbounded documents and overstated evidence   |
| CON-02  | Version 1 projects survive upgrade to the version 2 envelope and downgrade back unchanged, with coded loss diagnostics        |
| CON-03  | A verification claim must name issuer, target, verifier version and observation time; a source cannot declare itself verified |
| CON-04  | Projections are positive allowlists: only the human projection carries a destination URL or device code                       |
| CON-05  | Compatibility issues cannot report a security requirement as informational or a blocking issue with no execution impact       |
| CON-06  | The nine connector schema documents match generation from the runtime contracts and agree with them on shared fixtures        |
...
The CON cases live in [tests/connectors/contracts](../../tests/connectors/contracts). Connector structural drift is covered by CON-06 in addition to SPEC-10, which iterates every generated schema document.
