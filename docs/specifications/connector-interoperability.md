...
CIP-09a: A catalog entry's `supportLabel` (`unverified`, `fixture`, `local`, `live`, `certified`, weakest to strongest) MUST be computed from dated evidence entries by `computeSupportLabel`, never from the adapter family, a registration or a model's suggestion. An entry names one adapter, one check (a repository path or a `scheme:identifier`, never a URL), the target it ran against (`in-process-fixture`, `local-double`, `recorded-live`, `attended-live`) and the UTC day it was recorded; an `attended-live` entry MUST name who attended it. The label is the strongest one any fresh, admissible entry earns, with the minimum evidence and freshness windows in `supportLabelRules` (365 days for fixture and local evidence, 90 for a recorded live run, 180 for an attended certification). An expired entry earns nothing. `live` and `certified` evidence is admissible only where the configuration it was measured with is present. An entry dated after the evaluation day MUST be refused where entries are accepted (the ledger generator, a host's own entries) and MUST NOT count where labels are computed, and an evaluation instant that is not finite MUST be refused rather than compared. A legacy ledger work item that carries only an evidence level earns at most `fixture`, and a legacy live level MUST be refused.

CIP-09b: A generic adapter (`evidenceScope: "definition"`: the OpenAPI adapter and the unpinned provider-catalog adapter) runs whatever description a person imported, so an entry MAY name the one definition it exercised (`definition`: its `definitionRef` or `sha256:<normalizedDigest>`, never a URL). Evaluated for a definition, a generic adapter MUST count only entries naming that definition, so an imported description nobody exercised is `unverified`; a single-provider adapter counts its adapter-wide entries and those naming that definition. Evaluated adapter-wide (a catalog row, the published matrix, marked "code path"), an entry naming a definition MUST NOT count, and a generic adapter MUST NOT read `live` or `certified`, because no provider was named.

CIP-09c: Only a `provider-backed` entry may carry a `live` or `certified` label. A single-provider `fixture`-family adapter whose adapter-wide evidence earns `live` or `certified` with its configuration present is shown `provider-backed`; without that evidence it stays `fixture`, however well it is exercised locally, and the label says how well. A generic adapter is never promoted in the catalog. A registration is `live-adapter` only when the label of its own definition is `live` or `certified`.

CIP-09d: A label gates nothing unless the host opts in. With `support.minimumForProduction`, while the label of a binding's definition is below the minimum, a binding that can reach any destination other than a `loopback-fixture` (or names none) MUST be refused with `support.below-minimum` at every step that starts or continues work with the provider: approval, connect, completion of a pending ceremony (callback, private input, provider event), poll, verify, reconnect and invoke. An in-flight ceremony whose evidence lapsed therefore does not finish. Disconnect, revocation and deletion MUST NOT be gated.
