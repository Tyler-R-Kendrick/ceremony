...
This is protocol-fixture evidence against the loopback authorization server double and a fixture API; it is not certified against any provider. JWT assertion grants are not implemented. Remote MCP connections under the default profile renew their token once, single-flight, on invocation, on verification (which reconnect and polling reach, as does `connector_verify`) and on the continuation of a suspended input request; on verification only a renewal the issuer refuses leaves the connection needing a person (`reconnect-required`, `mcp.credential-renewal-failed`). An issuer that cannot be reached leaves it as it was, answering `pending` with `mcp.credential-renewal-unavailable` so it can be verified again later, and a refresh that was cancelled, raced another worker or lost its response is `indeterminate` (`mcp.credential-renewal-indeterminate`); a refresh token that may have been spent is never presented again. None of these carries an issuer message. Upstream revocation on disconnect runs only where the reviewed issuer policy asks for it.
