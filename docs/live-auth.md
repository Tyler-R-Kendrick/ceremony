...
Open [Live GitHub](http://127.0.0.1:4173/?mode=live&connector=github) after `npm run dev`. Other service examples remain explicitly local simulations. This implementation is not yet certified against a real GitHub account, Cloudflare account, A2H gateway or end-user MCP host.
...
Open [Environment](http://127.0.0.1:4173/?mode=live&section=environment). Select a connector, optionally import a `.env` file, or save a named value. Import merges assignments and replaces matching names; Remove deletes only that connector's selected variable. Values are masked during manual entry and are never returned by the metadata endpoint. To edit a saved value, enter its name and replacement. An empty value is allowed; removal is explicit.

The server uses Node's `.env` parser, including quoted multiline values, with no shell execution or variable expansion. Limits are 100 variables, 64 KB total, and 16 KB per value. Names must match `[A-Za-z_][A-Za-z0-9_]*`. Invalid edits and stale revisions are rejected. The file is not written to disk; parsed variables go into encrypted storage, scoped by session owner and connector. Losing the owner session loses UI access to that environment. The demo does not expose or modify `process.env`, vault keys, service origins or server credentials.

Live GitHub consumes `GITHUB_APP_ID`, `GITHUB_APP_SLUG`, `GITHUB_APP_OWNER`, and `GITHUB_APP_PRIVATE_KEY` together when setup begins. An incomplete set blocks setup with an actionable message; no set uses guided registration. Provider signature/identity/permission checks still apply. Already configured attempts retain their pinned app; changing Environment does not silently switch an in-progress authorization. Other connectors store variables for host adapters but their current local simulations do not consume them.

External adapters can use exported `CeremonyEnvironment.read(owner, connector)` on the server. `describe` returns names/revision only; `update` performs scoped encrypted edits with optimistic revisions. `GitHubOptions.resolveApp(owner)` supplies configuration at the preparation boundary. Environment editing is private configuration, not a model tool accepting raw secrets; WebMCP ceremony actions and hooks never receive the uploaded values. Hosts must exclude this native input page from DOM capture, session recording and analytics, just like private credential collectors.
...
`registerPrivateCollector` from `@ceremony/auth/server` registers official MCP Apps resources/tools on a host-authenticated MCP server. Supply `{ brokerOrigin, appOrigin, appHtml, owner(context) }`. Bundle `mountPrivateCollector(root, brokerOrigin)` from `@ceremony/auth/mcp-app` into trusted `appHtml`; do not use generated HTML. Mount the returned `handleRequest` at `https://<broker>/ceremony/private-collection`.

The tool sends only a collection grant in app-only metadata. The iframe posts native values directly over HTTPS, clears the form, and calls the binding tool with the reference. The broker accepts only the configured app origin, rejects opaque/null origins, and consumes each grant once. Unsupported MCP hosts fail closed and direct the human back to the authenticated webpage. The library does not install or configure a third-party MCP host.
