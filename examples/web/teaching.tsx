...
async function request<T>(
  ...
  if (!navigator.onLine && body !== undefined)
    throw new Error(
...
export function TeachingConnection({
  ...
  useEffect(() => {
    ...
    const tools = createConnectionTools(`ceremony_${connectorId}`, {
      connect: () =>
  ...
  useEffect(() => {
    ...
    request<{
      ...
      .then(async (value) => {
        ...
        await loadRecipes();
        const hint = new URL(location.href).searchParams.get("teachingRun");
        if (hint)
          remember(
            await request<Run>(
    ...
    if (sessionChannel)
      sessionChannel.onmessage = (event) => {
        if (event.data === "signed-out") location.replace("/");
      };
    return () => {
  ...
  }, [run, active, busy, offline, act, refresh]);
  useEffect(() => {
    if (draft) {
          ...
          <details>
            ...
            <div className="teaching-actions">
              {capabilities.modelAvailable && (
                ...
                >
                  {assistant ? "Stop assistant" : "Ask assistant to help"}
                </button>
              )}
              <button
                disabled={busy || offline}
