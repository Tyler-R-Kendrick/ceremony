...
The normal **Connect** page contains only provider-backed adapters. The explicit developer harness at `/?mode=test` retains GitHub OAuth/device/token, Stripe, Jira, Supabase and Neon simulations for protocol testing; never enter real credentials there. Jira and Neon are not offered as ready live integrations. See [service examples and coverage](docs/service-examples.md). **Workflow studio** customizes presentation templates and exports trusted Arazzo definitions; it does not create a second connection experience or execute generated code.
...
To verify registration, open **Application → WebMCP → Available tools** on the Connect page. The selected connector exposes eleven tools (for example, `ceremony_github_read`). Changing connectors replaces that tool set. Tests verify `WebMCP.toolsAdded`, `WebMCP.toolsRemoved` and `WebMCP.invokeTool` through the same [DevTools protocol](https://chromedevtools.github.io/devtools-protocol/tot/WebMCP/) used by the panel, including opening DevTools after page load. They use native Chromium 153 with experimental web-platform features enabled; the navigator-only compatibility case aliases that native registry, rather than claiming a test against an installed older Chrome release. If neither browser API exists, enable Chrome's WebMCP feature and reload on a secure origin (HTTPS or localhost).
