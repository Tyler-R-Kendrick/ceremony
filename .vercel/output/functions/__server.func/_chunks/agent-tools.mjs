...
import { number, strictObject, string } from "../_libs/@ai-sdk/gateway+[...].mjs";
import { AuthorizationError, requireCapability } from "./runtime.mjs";
...
*/
const identifier = string().regex(/^[a-zA-Z][a-zA-Z0-9_.:-]{0,119}$/);
const revision = number().int().positive();
const agentToolInputs = {
	connect: strictObject({ connectorId: identifier }),
	snapshot: strictObject({ runId: identifier }),
	advance: strictObject({
		runId: identifier,
		nodeId: identifier,
		revision,
...
function ceremonyAgentTools(runtime) {
	/**
	...
	*/
	async function delegatedFor(actor, runId) {
		await runtime.commands.snapshot(actor, runId);
		const delegated = await runtime.agentActor(runId);
		if (delegated.tenantId !== actor.tenantId || delegated.subjectId !== actor.subjectId || delegated.sessionId !== actor.sessionId) throw new AuthorizationError("denied");
		return delegated;
	}
	return {
		async connect(actor, input) {
			...
			for (const node of run.nodes) {
				...
				const result = await runtime.commands.advance(delegated.actor, run.id, node.id, run.revision, `native:${run.id}:${node.id}:${run.revision}`);
				run = await runtime.commands.snapshot(delegated.actor, run.id);
				if (result.state !== "complete") break;
			}
			return run;
		},
		async snapshot(actor, input) {
			requireCapability(actor, "executor");
			const { runId } = agentToolInputs.snapshot.parse(input);
			return await runtime.commands.snapshot(actor, runId);
		},
		async advance(actor, input) {
			requireCapability(actor, "executor");
			const checked = agentToolInputs.advance.parse(input);
			const delegated = await delegatedFor(actor, checked.runId);
			await runtime.commands.advance(delegated, checked.runId, checked.nodeId, checked.revision, checked.commandId);
